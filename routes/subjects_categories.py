...
class CategoryOfSubject:

    @classmethod
    @jwt_required_with_redirect(admin=True)
    def category_of_subject(cls, name: str):
        try:
            subject = SubjectModel.find_by_name(name)
        except NoResultFound:
            return {"message": SUBJECT_DOESNT_EXIST.format(name)}, 404
        categories = subject_category_schema.load(
            request.json["subject_categories"], many=True
        )
        for category in categories:
            try:
                SubjectCategoryModel.find_by_name(category.name)
            except NoResultFound:
                return {"message": SUBJECT_CATEGORY_DOESNT_EXIST.format(
                    category.name)}, 400
    ...
    @classmethod
    @jwt_required_with_redirect(admin=True)
    def categories_of_subject(cls, name: str):
        try:
            subject = SubjectModel.find_by_name(name)
        except NoResultFound:
            return {"message": SUBJECT_DOESNT_EXIST.format(name)}, 404
        return {
            "subject_categories": subject_category_schema.dump(
                subject.categories, many=True
            )
        }, 200
